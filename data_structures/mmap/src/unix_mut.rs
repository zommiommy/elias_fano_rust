...
impl MemoryMapped {
    ...
    pub fn new(path: Option<&str>, len: Option<usize>) -> Result<Self> {
        let (addr, fd, len) = match (path, len) {
            ...
            (Some(path), maybe_len) => {
                let (fd, len) = if let Some(len) = maybe_len {
                    ...
                    if res == -1 {
                        unsafe{close(fd)};
                        return Err(
                            format!("Cannot ftruncate the file '{}'", 
                            path)
                        );
                    }
                    (fd, len)
            ...
            (None, None) => {
                return Err(
                    "Cannot create an mmap without both a path and a len"
                    .to_string()
                );
            }
        };
        ...
        Ok(MemoryMapped{ fd, addr, len, path: path.map(|x| x.into()) })
    }

    pub fn sync_flush(&self) -> Result<()> {
        if self.fd.is_some() {
            unsafe {
    ...
    pub fn get_slice<T>(&self, offset: usize, elements_len: Option<usize>) -> Result<&[T]> {
        ...
        if bytes_len > self.len - offset {
            return Err(format!(concat!(
                ...
                ), self.len, offset, self.len - offset, 
                elements_len, std::mem::size_of::<T>(), bytes_len,
                (self.len - offset) - bytes_len,
            ));
        }

        Ok(unsafe {
            self.get_slice_unchecked(offset, Some(elements_len))
        })
    }
    ...
    /// Returns a new slice of `len` object of type `T` starting from `offset`
    /// bytes from the start of the memory.
    pub fn get_slice_mut<T>(&mut self, offset: usize, elements_len: Option<usize>) -> Result<&mut [T]> {
        let elements_len = elements_len.unwrap_or(
            self.len().saturating_sub(offset) / std::mem::size_of::<T>()
        );
        // Convert from number of elements to number of bytes
        ...
        Ok(unsafe {
            self.get_slice_mut_unchecked::<T>(offset, Some(elements_len))
        })
    }

    /// Returns a new slice of `len` object of type `T` starting from `offset`
    /// bytes from the start of the memory.
    pub unsafe fn get_slice_mut_unchecked<T>(&mut self, offset: usize, elements_len: Option<usize>) -> &mut [T] {
        let elements_len = elements_len.unwrap_or(
            self.len().saturating_sub(offset) / std::mem::size_of::<T>()
        );
    ...
    /// # Safety
    /// This assumes that the data is valid utf8 chars.
    pub fn as_str_mut(&mut self, offset: usize, len: Option<usize>) -> Result<&mut str> {
        unsafe{
            Ok(
