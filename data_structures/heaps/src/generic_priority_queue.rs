...
impl GenericPriorityQueue {
    ...
    pub fn push(&mut self, value: Element) {
        ...
        loop {
            let parent_idx = GenericPriorityQueue::parent(idx);
    ...
    pub fn pop(&mut self) -> Option<Element> {
        ...
        loop {
            ...
            let left_i = GenericPriorityQueue::left(idx);
            let right_i = GenericPriorityQueue::right(idx);
            let left_v = self.values.get(left_i).map(|x| *x).unwrap_or(Element::default());
            let right_v = self.values.get(right_i).map(|x| *x).unwrap_or(Element::default());
