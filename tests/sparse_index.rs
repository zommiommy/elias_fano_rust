...
fn test_simple_select_against_fid() {
    ...
    for i in 0..rs.rank1(rs.len()) {
        assert_eq!(hb.select1(i as usize) as u64, rs.select1(i), "error seleting the {}-th one", i);
    }

    for i in 0..rs.rank0(rs.len()) {
        assert_eq!(hb.select0(i as usize) as u64, rs.select0(i), "error seleting the {}-th zero", i);
    }

    for i in 0..rs.len() {
        assert_eq!(hb.rank1(i as usize) as u64, rs.rank1(i), "error ranking ones up to {}", i);
    }

    for i in 0..rs.len() {
        assert_eq!(hb.rank0(i as usize) as u64, rs.rank0(i), "error ranking zeros up to {}", i);
    }
    ...
    println!("Simple select uses: {} Mib", hb.total_size() as f64 / (1024.0*1024.0));
